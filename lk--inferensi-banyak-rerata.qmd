...
abstract: |
  Banyak permasalah statistik yang bertujuan untuk membandingkan tiga atau lebih rerata populasi ketika diketahui rerata sampelnya. Artikel ini bertujuan untuk mendemonstrasikan penggunaan uji ANOVA untuk membandingkan banyak rerata serta memberikan penjelasan logika di balik metode statistik tersebut secara ringkas, termasuk dengan demonstrasi pemeriksaan kondisi-kondisi atau asumsi-asumsi yang diperlukan sebelum melakukan uji ANOVA tersebut. Artikel ini juga menjelaskan bagaimana melakukan uji perbandingan berganda Tukey yang digunakan untuk mendeteksi pasangan rerata mana yang berbeda. Selain itu, uji Kruskal-Wallis yang merupakan uji nonparametrik yang menjadi alternatif dari uji ANOVA juga dijelaskan secara singkat dan didemonstrasikan dalam artikel ini. Semua uji hipotesis tersebut didemonstrasikan dengan menggunakan pemrograman R.
date: 2023/11/28
author:
...
format:
  html:
    ...
    keywords: "normalitas, homogenitas variansi, uji hipotesis, ANOVA, Kruskal-Wallis, Tukey, R"
    page-layout: article
    code-fold: true
    code-tools: true
...
Jika dibandingkan dengan 'teman-temannya' pada @fig-samp-sim-buku, apakah sampel yang kita miliki, yaitu `sampel_buku` yang distribusinya ditunjukkan pada @fig-violin-harga-buku, merupakan sampel yang biasa atau sampel yang hampir mustahil untuk diperoleh?
...
Ternyata ada 10 dari 10.000 sampel hasil simulasi yang statistik $F$-nya lebih dari atau sama dengan statistik $F$ `sampel_buku`. Artinya, dengan mengasumsikan bahwa rerata harga bukunya sama di setiap kategorinya, kita mendapatkan bahwa peluang untuk mendapatkan sampel yang perbedaan rerata `harga` buku di setiap `kategori`-nya seekstrem atau lebih ekstrem dari `sampel_buku` yang kita miliki sangatlah kecil, yaitu sekitar 0,01!
...
Ketika statistik $F$ dari semua kemungkinan sampel yang berukuran $n$ dan memiliki $k$ kelompok dihimpun jadi satu, kita mendapatkan distribusi sampling statistik $F$. Jika hipotesis nol diasumsikan benar dan sampel yang kita miliki memenuhi kondisi-kondisi tertentu, maka distribusi sampling tersebut bentuknya mengikuti distribusi $F$. Distribusi $F$ ini memiliki dua parameter, yaitu $\text{df}_1 = k - 1$ dan $\text{df}_2 = n - k$.

Misalnya, jika kita memiliki sampel yang berukuran $n = 30$ dan sampel tersebut memuat $k = 3$ kelompok (seperti `sampel_buku` yang kita gunakan sebelumnya), maka distribusi $F$-nya memiliki bentuk seperti yang ditunjukkan pada @fig-dist-F-2-27 berikut.
...
Berdasarkan @fig-QQ-kategori-buku tersebut, kita dapat mengasumsikan bahwa data `harga` buku di setiap `kategori`-nya berasal dari populasi yang berdistribusi normal. Kesimpulan ini dapat didukung oleh hasil uji Shapiro-Wilk seperti yang ditunjukkan pada @tbl-hasil-Shapiro-Wilk-harga-buku.
...
sampel_buku %>% 
  group_by(kategori) %>% 
  summarise(W = shapiro.test(harga)$statistic,
            p = shapiro.test(harga)$p.value) %>% 
  kbl(linesep = "",
      booktabs = TRUE) %>% 
  kable_styling(bootstrap_options = c("striped",
                                      "hover",
...
Untuk melakukannya, kita dapat menggunakan perintah `leveneTest()` yang merupakan salah satu fungsi dari paket {car}.

```{r}
#| code-fold: false

leveneTest(harga ~ kategori, sampel_buku)
```
...
Sampel yang kita miliki, yaitu `sampel_buku`, berukuran $n = 30$ dan memiliki $k = 3$ kategori, maka distribusi $F$ yang kita gunakan memiliki $\text{df}_1 = 3 - 1 = 2$ dan $\text{df}_2 = 30 - 3 = 27$. Distribusinya ditunjukkan pada @fig-dist-F-2-27.
...
Berdasarkan perhitungan itu kita peroleh nilai-*P* yang kurang lebih `r round(P_buku, 5)`.
...
Terakhir, kita juga telah mempelajari uji hipotesis alternatif dari uji ANOVA, yaitu uji Kruskal-Wallis. Uji nonparametrik ini tidak memperlukan dua kondisi yang dipersyaratkan oleh ANOVA, yaitu tentang normalitas dan homogenitas variansi.
